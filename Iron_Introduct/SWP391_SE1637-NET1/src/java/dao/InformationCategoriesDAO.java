...
public class InformationCategoriesDAO {
    ...
    private static final String GET_PINFORCATE_BY_ID = "select * from swp_gr01.informationcategories WHERE pId = ?";
    private static final String INSERT_INFORCATE = "insert into swp_gr01.informationcategories (name, pId, status) values (?,?,?)";
    private static final String UPDATE_INFORCATE = "update swp_gr01.informationcategories set name=?,pId=?,status=? where id = ?";
    ...
    public List<InformationCategories> getInforCateByPID(int pid) throws SQLException {
        ...
        try {
            conn = DBContext.getJDBCConnection();
            if (conn != null) {
                stm = conn.prepareStatement(GET_PINFORCATE_BY_ID);
                stm.setInt(1, pid);
                rs = stm.executeQuery();
                while (rs.next()) {
                    int id = rs.getInt("id");
                    ...
                    listCate.add(new InformationCategories(id,name,pId));
                }
            }
        } catch (Exception e) {
        } finally {
        ...
        try {
            ...
            if (conn != null) {
                ...
                while (rs.next()) {
                    ...
                    listCate.add(new InformationCategories(id,name,pId,status));
                }
            }
        } catch (Exception e) {
        } finally {
    ...
    public boolean insertInforCate(String name, int pId, boolean status) throws SQLException{
        boolean check = false;
        Connection conn = null;
        PreparedStatement stm = null;
    ...
    public boolean updateInforCate(String name,int pId,boolean status,int id) throws SQLException{
        boolean check = false;
        Connection conn = null;
        PreparedStatement stm = null;
        try {
