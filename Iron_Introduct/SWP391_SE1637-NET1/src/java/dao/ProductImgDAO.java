...
public class ProductImgDAO {
    ...
    private static final String SELECT_IMG_LINK = "SELECT imgPath FROM swp_gr01.product_img WHERE pId = ?";
...
//                stm.setString(1, imgPath);
    ...
    public List<ProductImg> getProductImgLink(int pid) throws SQLException {
        List<ProductImg> list = new ArrayList<>();
        Connection conn = null;
        PreparedStatement psm = null;
        ResultSet rs = null;
        try {
            ...
            if (conn != null) {
                ...
                psm = conn.prepareStatement(SELECT_IMG_LINK);
                psm.setInt(1, pid);
                rs = psm.executeQuery();
                if (rs != null) {
                    while (rs.next()) {
                        ...
                        ProductImg productimg = new ProductImg(pid, pid, imgPath);
                        list.add(productimg);
                    }
                }
            }
...
//        }
        ...
        // test getProduct
            String basePath = "C:\\Users\\lenam\\OneDrive\\Desktop\\swp_finalv3\\SWP391_SE1637-NET1\\web\\files";

        // test getProduct
        ProductImgDAO p = new ProductImgDAO();
        for (ProductImg productImg : p.getProductImgLink(108)) {
            String fileName = productImg.getImgPath();
