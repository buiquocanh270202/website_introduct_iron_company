...
public class UpdateInforCateController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            //List<InformationPCategories> pCate = new InformationPCategoriesDAO().getAllPInforCate();
            ...
            if (action != null) {
                if (action.equals("insertInforCate")) {
                    ...
                    if (valid.checkInformationCateExist(list, inforCate)) {
                        ...
                        boolean check1 = cateDAO.insertInforCate(inforCate, pId, status);
                        if (check1) {
                    ...
                    boolean status = Boolean.parseBoolean(request.getParameter("status"));
                    List<InformationCategories> listCategories = cateDAO.getAllInforCate();
                    boolean check1 = cateDAO.updateInforCate(cateName,pId,status ,cateId);
                    if (check1) {
                        session.setAttribute("listCategories", listCategories);
...
//                else if (action.equals("updateInforPCate")) {
//                    int pcateId = Integer.parseInt(request.getParameter("pId"));
//                    String pname = request.getParameter("pName");
//                    boolean status = Boolean.parseBoolean(request.getParameter("Status"));
//                    InformationPCategoriesDAO pDAO = new InformationPCategoriesDAO();
//                    boolean check1 = pDAO.updatePInforCate(pname, status, pcateId);
//                    if (check1) {
//                        List<InformationCategories> listCategories = cateDAO.getAllInforCate();
//                        session.setAttribute("listCategories", listCategories);
//                        request.setAttribute("MSG_SUCCESS", "You have successfully updated the Pcategory information!");
//                    } else {
//                        request.setAttribute("MSG_ERROR", "You have failed to update category information!");
//                    }
//                } 
//                else if (action.equals("insertInforPCate")) {
//                    String pname = request.getParameter("pName");
//                    boolean status = Boolean.parseBoolean(request.getParameter("Status"));
//                    InformationPCategoriesDAO pDAO = new InformationPCategoriesDAO();
//                    boolean check1 = pDAO.insertPInforCate(pname, status);
//                    if (check1) {
//                        List<InformationCategories> listCategories = cateDAO.getAllInforCate();
//                        session.setAttribute("listCategories", listCategories);
//                        request.setAttribute("MSG_SUCCESS", "You have successfully insert the Pcategory information!");
//                    } else {
//                        request.setAttribute("MSG_ERROR", "You have failed to insert category information!");
//                    }
//                }
            } else {
