...
<html lang="en">
    <head>
        <title>VietNam Shoring&System Scaffolding Equipment Parts Supplier Detail</title>
                    ...
                    <div class="wrapper row">
                        <div class="preview col-md-6">
                            ...
                            <div class="preview-pic tab-content">
                                <div class="tab-pane active" id="pic-1"><img src="/SWP391_SE1637-NET/${detail.imgPath}" alt="${detail.imgPath}"  style="height: 340px; width: 510px"/></div>
                                    <c:forEach items="${requestScope.listProImg}" var="LPI" varStatus="loop">
                                        <c:if test="${LPI.pId eq detail.pId}">
                                        <div class="tab-pane" id="pic-${loop.index + 1}"><img  src="/SWP391_SE1637-NET/${LPI.imgPath}" alt="${LPI.imgPath}" style="height: 340px; width: 510px" /></div>
                                        </c:if>
                                    </c:forEach>
                            </div>
                            <ul class="preview-thumbnail nav nav-tabs">
                                <li class="active"><a data-target="#pic-1" data-toggle="tab"><img src="/SWP391_SE1637-NET/${detail.imgPath}" alt="${detail.imgPath}" style="height: 60px; width: 90px" /></a></li>
                                        <c:forEach items="${requestScope.listProImg}" var="LPI" varStatus="loop">
                                            <c:if test="${LPI.pId eq detail.pId}">
                                        <li><a data-target="#pic-${loop.index + 1}" data-toggle="tab"><img src="/SWP391_SE1637-NET/${LPI.imgPath}" alt="${LPI.imgPath}" style="height: 60px; width: 90px"/></a></li>
                                            </c:if>
                                        </c:forEach>
                            </ul>
                        ...
                        <div class="details col-md-6">
                            ...
                            <h5 class="sizes">Category:
                                <c:forEach items="${requestScope.listCate}" var="LC">  
                                    <c:if test="${LC.cateId eq detail.cateId}"> <a href="/SWP391_SE1637-NET/HomeCategoryController?cateId=${detail.cateId}">${LC.cateName}</a></c:if>
                                </c:forEach>
                            ...
                            <div class="action" style="display:flex; gap: 1%">
                                <a onclick="addToCartAsync(${detail.pId})" class="btn btn-outline-dark mt-auto w-50 like btn btn-default">Add To Cart</a>
                                <form action="/SWP391_SE1637-NET/DownloadFileController" method="get">
                                    <input  value="${detail.detailPath}" name="detailPath" hidden="">
                                    <button type="submit" class="like btn btn-default">Catalogue</button>
                                </form>
                            </div>
                                        ...
                                        <c:if test="${LP.cateId eq detail.cateId and (LP.status eq 1) and !(LP.pName eq detail.pName)}">
                                            <div class="owl-item">
                                                <div class="bbb_viewed_item discount d-flex flex-column align-items-center justify-content-center text-center">
                                                    <div class="bbb_viewed_image"><a href="/SWP391_SE1637-NET/HomeProductDetailController/${LP.url}" class="normal-font"><img src="/SWP391_SE1637-NET/${LP.imgPath}"  class="img-responsive" style="height: 100px; width: 150px" ></a></div>
                                                    <div class="bbb_viewed_content text-center">

                                                        <div class="bbb_viewed_name"><a href="/SWP391_SE1637-NET/HomeProductDetailController/${LP.url}">${LP.pName}</a></div>
                                                    </div>
                                                </div>
                                            </div>
                                        </c:if>
...
// Lặp qua các tab thumbnail và đăng ký sự kiện click
    ...
    <script>
        ...
        function addToCartAsync(pId) {
            ...
            axios.get('http://localhost:9999/SWP391_SE1637-NET/AddToCartController', {
